...
### 1. [`message.mbt`](file:///d:/yiduo/runtime/core/message.mbt)
- **Int ID替代String** → 体现了"WIT→Int ID"的运行期优化
- 移除了字符串解析开销 → 极致性能

### 2. [`scheduler.mbt`](file:///d:/yiduo/runtime/scheduler/scheduler.mbt)
- **纯Int ID调度** → 实现了运行期的"机器层"
- 消息传递用Int ID → O(1)查找

### 3. [`memory_pool.mbt`](file:///d:/yiduo/runtime/core/memory_pool.mbt)
