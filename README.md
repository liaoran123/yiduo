...
| 四元 | 作用 | 对应实现 |
|-----|------|----------|
| **💭 意志（配置文件）** | 造物主的一念之间，定义"我需要什么" | `configs/` - 用户通过声明式配置表达需求 |
| **🧬 器官（组件）= DNA** | 自带遗传信息的功能单元，可自由组合 | `components/` - 所有功能都是独立的组件 |
| **🌳 树形（生长路径）** | DNA 按意志组合成的树形结构 | 配置文件中的组件层级关系 |
| **🩸 链式（血液系统）** | 贯穿整个系统的血液，保证数据流通 | `interfaces/` - 稳定不变的接口骨架 |
